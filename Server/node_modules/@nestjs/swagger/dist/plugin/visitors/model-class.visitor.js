...
const external_imports_util_1 = require("../utils/external-imports.util");
...
class ModelClassVisitor extends abstract_visitor_1.AbstractFileVisitor {
    ...
    visit(sourceFile, ctx, program, options) {
        var _a;
        const externalImports = (0, external_imports_util_1.getExternalImports)(sourceFile);
        ...
        const propertyNodeVisitorFactory = (metadata) => (node) => {
            const visit = () => {
                if (ts.isPropertyDeclaration(node)) {
                    this.visitPropertyNodeDeclaration(node, ctx, typeChecker, options, sourceFile, metadata, externalImports, compilerOptionsPaths);
                }
                else if (options.parameterProperties &&
                    ts.isConstructorDeclaration(node)) {
                    this.visitConstructorDeclarationNode(node, typeChecker, options, sourceFile, metadata, externalImports, compilerOptionsPaths);
                }
                return node;
            };
    ...
    visitPropertyNodeDeclaration(node, ctx, typeChecker, options, sourceFile, metadata, externalImports, compilerOptionsPaths) {
        ...
        try {
            this.inspectPropertyDeclaration(ctx.factory, node, typeChecker, options, sourceFile.fileName, sourceFile, metadata, externalImports, compilerOptionsPaths);
        }
        catch (err) {
            return node;
        }
    }
    visitConstructorDeclarationNode(constructorNode, typeChecker, options, sourceFile, metadata, externalImports, compilerOptionsPaths) {
        constructorNode.forEachChild((node) => {
            if (ts.isParameter(node) &&
                ...
                const objectLiteralExpr = this.createDecoratorObjectLiteralExpr(typescript_1.factory, node, typeChecker, typescript_1.factory.createNodeArray(), options, sourceFile.fileName, sourceFile, externalImports, compilerOptionsPaths);
    ...
    inspectPropertyDeclaration(factory, compilerNode, typeChecker, options, hostFilename, sourceFile, metadata, externalImports, compilerOptionsPaths) {
        const objectLiteralExpr = this.createDecoratorObjectLiteralExpr(factory, compilerNode, typeChecker, factory.createNodeArray(), options, hostFilename, sourceFile, externalImports, compilerOptionsPaths);
        this.addClassMetadata(compilerNode, objectLiteralExpr, sourceFile, metadata);
    }
    createDecoratorObjectLiteralExpr(factory, node, typeChecker, existingProperties = factory.createNodeArray(), options = {}, hostFilename = '', sourceFile, externalImports = {}, compilerOptionsPaths = {}) {
        ...
        const properties = [
            ...
            ...this.createTypePropertyAssignments(factory, node.type, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths),
            ...
            this.createEnumPropertyAssignment(factory, node, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths)
        ];
    ...
    createTypePropertyAssignments(factory, node, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths) {
        const key = 'type';
        ...
        if (node) {
            if (ts.isTypeLiteralNode(node)) {
                const initializer = this.createInitializerForTypeLiteralNode(node, factory, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths);
                ...
                if (remainingTypes.length === 1) {
                    const propertyAssignments = this.createTypePropertyAssignments(factory, remainingTypes[0], typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths);
                    if (!isNullable) {
    ...
    createInitializerForTypeLiteralNode(node, factory, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths) {
        const propertyAssignments = Array.from(node.members || []).map((member) => {
            const literalExpr = this.createDecoratorObjectLiteralExpr(factory, member, typeChecker, existingProperties, options, hostFilename, undefined, externalImports, compilerOptionsPaths);
    ...
    createEnumPropertyAssignment(factory, node, typeChecker, existingProperties, hostFilename, options, externalImports, compilerOptionsPaths) {
        const key = 'enum';
        if ((0, plugin_utils_1.hasPropertyKey)(key, existingProperties)) {
            return undefined;
        }
        ...
        const typeReferenceDescriptor = {
            typeName: (0, external_imports_util_1.replaceExternalImportsInTypeReference)((0, ast_utils_1.getText)(type, typeChecker), externalImports)
        };
        const enumIdentifier = (0, type_reference_to_identifier_util_1.typeReferenceToIdentifier)(typeReferenceDescriptor, hostFilename, options, factory, type, this._typeImports, compilerOptionsPaths);
