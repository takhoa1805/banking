...
export function WithdrawForm({ className, accountId, ...props }: React.ComponentPropsWithoutRef<'div'>) {
  ...
  const onSubmit = async (data) => {
    ...
    await axios
      .post(
        'http://localhost:3000/transactions/withdrawal',
        {
          type: 'WITHDRAWAL',
      ...
      .catch((err) => {
        setIsActive({ state: true, message: 'Lỗi xảy ra thực hiện rút tiền' });
        reset();
      });
  };
  return (
